# Migrating Expo UI to NOW Design System

Read `artifacts/now-design-system/docs/AGENTS.md` and
`artifacts/now-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/now-design-system/lib/native-theme` and
  `@workspace/now-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/now-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/now-design-system` and retains only
