# Consuming NOW Design System in web apps

Read `artifacts/now-design-system/docs/AGENTS.md` first. This guide covers
...
`artifacts/now-design-system/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/now-design-system/styles.css";
```
...
`node_modules/@workspace/now-design-system/src/components` to `content`.
...
import {
  toast,
  useToast,
} from "@workspace/now-design-system/hooks/use-toast";
```
...
`@workspace/now-design-system/components/ui/button`. Run the app's typecheck
