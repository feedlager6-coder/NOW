# NOW Design System design system
...
workspace package (`@workspace/now-design-system`): other artifacts depend
...
- `src/components/ui/` — the themed primitive library used by NOW. The current
  source-backed pilot covers Button, Card, Badge, Input, and Switch; product
  compositions such as meetup cards and City Lights stay in consuming apps.
...
- `docs/references/component-inventory.md` — the source-backed component
  inventory and UX architecture for NOW.
...
`@workspace/now-design-system/components/...`. Never use a `@/` alias inside
...
for `@workspace/now-design-system` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/now-design-system` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/now-design-system/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/now-design-system/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/now-design-system/docs/consuming-slides.md`
...
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/now-design-system/docs/migrating-expo.md`
...
- Match exact token values. Do not invent colors, fonts, spacing, or radii in a
  consuming app.
- NOW is mobile-first: prioritize one-thumb reach, one primary action per
  surface, anonymous activity signals, and public-safety clarity.
- Use electric blue only for attention and action. Warm amber/orange is reserved
  for City Lights activity intensity and live status, never as a full-page fill.
- Motion uses 120ms for press feedback, 220ms for state changes, 350ms for
  sheets/cards, and 500ms for hero moments. Respect reduced-motion settings.
